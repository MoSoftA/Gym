        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th :key='index' v-for="(header, index) in head" v-if="index!=3">{{ header }}</th>
                            <th>Options</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr :key='index' v-for="(user, index) in rows">
                            <td :key='index' v-for="(val, index) in user" v-if="index!=3">{{ val }}</td>
                            <td>
                                <button class="btn btn-success" title="Edit Article"
                                    @click="change_component($event, 'articleEdit')"><i class="fas fa-edit"></i></button>

                                <button class="btn btn-danger" id="id" title="Delete Article" data-toggle="modal" data-target="#exampleModalCenter" @click="get_id($event)"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <!-- /.card-body -->
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                ...
                <div class="modal-body">
                        ...
                        <button class="btn btn-danger btn-flat my-4 mx-auto" data-dismiss="modal" type="button" id="button-addon2" @click="deleteArticle($event)">Just Do It</button>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteArticle(e) {
                axios.delete(`api/deleteArticle/${this.id}`,
                    {
                        headers: {
